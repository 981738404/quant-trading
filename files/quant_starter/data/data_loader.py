...
class DataLoader:
    ...
    def get_daily(
        ...
        Raises:
            ValueError: start > end 时
        """
        # 日期校验（YYYYMMDD 字符串可直接字典序比较）
        start_str = to_tushare_date(start)
        end_str = to_tushare_date(end)
        if start_str > end_str:
            raise ValueError(f"start ({start_str}) 必须 <= end ({end_str})")

        src = source or self.primary
        cache_key = self._cache_key(code, start_str, end_str, adjust, src)
        cache_path = self.cache_dir / f"{cache_key}.parquet"

        # 缓存命中
        if self.use_cache and cache_path.exists():
            logger.debug("缓存命中: %s", cache_path.name)
            return pd.read_parquet(cache_path)

        logger.debug("请求 %s %s %s→%s adj=%s", src, code, start_str, end_str, adjust)

        # 获取数据
        if src == "tushare":
            df = self.tushare.get_daily(code, start_str, end_str, adjust=adjust)
        elif src == "akshare":
            df = self.akshare.get_daily(code, start_str, end_str, adjust=adjust)
        else:
        ...
        if self.use_cache and not df.empty:
            ...
            logger.debug("写入缓存: %s", cache_path.name)
